...
func test_can_create_all_elements() -> void:
	skill = Ing.load_ingress([Ingress.Id.PIERCING_INCURSION, Element.Type.ETH])
	assert_not_null(skill)
	# skill = Ing.load_ingress([Ingress.Id.INCURSION, Element.Type.ENH])
	# assert_not_null(skill)
	skill = Ing.load_ingress([Ingress.Id.PIERCING_INCURSION, Element.Type.SHOR])
	assert_not_null(skill)
...
func test_can_process_correctly() -> void:
	...
	gut.p("----Test can pierce refrain----")
	mocker.reset_members_ingress()
	mocker.action.actor = mocker.player
	mocker.action.target = mocker.enemy
	mocker.action.target.modifiers.has_small_refrain_open = true
	mocker.action.target.modifiers.current_refrain_element = Element.Type.ENH
	await skill.process(mocker.action, get_tree(), mocker.battle_groups)
	assert_eq(
		mocker.player.current_ingress(),
		_remaining_ingress(mocker.MAX_INGRESS, skill.ingress),
