...
func test_can_create_all_elements() -> void:
	skill = Ing.load_ingress([Ingress.Id.DOUBLE_INCURSION, Element.Type.ETH])
	assert_not_null(skill)
	# skill = Ing.load_ingress([Ingress.Id.DOUBLE_INCURSION, Element.Type.ENH])
	# assert_not_null(skill)
	skill = Ing.load_ingress([Ingress.Id.DOUBLE_INCURSION, Element.Type.SHOR])
	assert_not_null(skill)
	# skill = Ing.load_ingress([Ingress.Id.DOUBLE_INCURSION, Element.Type.SCOR])
	# assert_not_null(skill)
...
func test_can_process_correctly() -> void:
	...
	assert_eq(mocker.enemy.current_ingress(), 0, "Enemy Ingress")
